...
abc2svg.cs_sel0=/[\[(].*[\])]/g
abc2svg.cs_sel1=/.*[\[(]|[\])].*/g
abc2svg.cs_filter=function(a_cs,sel){var i,cs,tcs=""
...
if(!p){error(1,s,"Bad position in !$1!",nm)
return}
ty=p[0]}
...
function draw_repbra(p_voice){var s,s1,x,y,y2,i,p,w,wh,first_repeat;y=staff_tb[p_voice.st].topbar+25
...
if(y<y2)
y=y2
if(s1.rbstart==2){y2=y_get(p_voice.st,true,s1.x,3)+20
if(y<y2)
y=y2}
if(s.rbstop==2){y2=y_get(p_voice.st,true,s.x-3,3)+20
if(y<y2)
y=y2}
...
var STEM_MIN=16,STEM_MIN2=14,STEM_MIN3=12,STEM_MIN4=10,STEM_CH_MIN=14,STEM_CH_MIN2=10,STEM_CH_MIN3=9,STEM_CH_MIN4=9,BEAM_DEPTH=3.2,BEAM_OFFSET=.25,BEAM_SHIFT=5,BEAM_STUB=8,SLUR_SLOPE=.5,GSTEM=15,GSTEM_XOFF=2.3
var cache,anno_a=[]
...
y1=bm.a*x1+bm.b-dy;x2-=x1;x2/=stv_g.scale;dy2=bm.a*x2
...
function draw_mrest(s){var x1,x2,p_st=staff_tb[s.st],y=p_st.y+(p_st.topbar+p_st.botbar)/2,p=s.nmes.toString()
...
note.sls=nsls}}}
s=s.next}}
function draw_tuplet(s1){var s2,s3,g,upstaff,nb_only,x1,x2,y1,y2,xm,ym,a,s0,yy,yx,dy,a,dir,r,w,tp=s1.tp.shift()
if(!s1.tp.length)
delete s1.tp
upstaff=s1.st
set_dscale(s1.st)
...
function draw_systems(indent){var s,s2,st,x,x2,res,sy,staves_bar,bar_force,xstaff=[],stl=[],bar_bot=[],bar_height=[],ba=[],sb="",thb=""
...
function draw_staff(st,x1,x2){var w,ws,i,dy,ty,y=0,ln="",stafflines=staff_tb[st].stafflines,l=stafflines.length
if(!/[\[|]/.test(stafflines))
return
...
xstaff[st]=-1}
bar_set()}
switch(s.type){case C.STAVES:if(s.ts_next.type==C.CLEF){staves_bar=s.x-s.ts_next.wl}else if(s.ts_prev.type==C.BAR){staves_bar=s.ts_prev.x}else{for(s2=s.ts_next;s2;s2=s2.ts_next){if(s2.time!=s.time)
break
...
if(x<0){if(sy.st_print[st]){xstaff[st]=staves_bar?staves_bar:(s.x-s.wl-2)
stl[st]=true}
continue}
...
N "Notes: "\n\
Z "Transcription: "\n\
H "History: "',infospace:0,keywarn:true,leftmargin:1.4*CM,lineskipfac:1.1,linewarn:true,maxshrink:.65,maxstaffsep:2000,maxsysstaffsep:2000,measrepnb:1,measurefont:{name:txt_ff,style:"italic",size:10},measurenb:-1,musicfont:{name:"music",src:musicfont,size:24},musicspace:6,partsfont:{name:txt_ff,size:15},parskipfac:.4,partsspace:8,pagewidth:21*CM,printmargin:0,rightmargin:1.4*CM,rbmax:4,rbmin:2,repeatfont:{name:txt_ff,size:13},scale:1,slurheight:1.0,spatab:new Float32Array([10.2,13.3,17.3,22.48,29.2,38,49.4,64.2,83.5,108.5]),staffsep:46,stemheight:21,stretchlast:.25,stretchstaff:true,subtitlefont:{name:txt_ff,size:16},subtitlespace:3,sysstaffsep:34,tempofont:{name:txt_ff,weight:"bold",size:12},textfont:{name:txt_ff,size:16},textspace:14,tieheight:1.0,titlefont:{name:txt_ff,size:20},titlespace:6,titletrim:true,topspace:22,tuplets:[0,0,0,0],tupletfont:{name:txt_ff,style:"italic",size:12},vocalfont:{name:txt_ff,weight:"bold",size:13},vocalspace:10,voicefont:{name:txt_ff,weight:"bold",size:13},writefields:"CMOPQsTWw",wordsfont:{name:txt_ff,size:16},wordsspace:5}
...
continue}
self.do_begin_end(b,uncomment(a[2]),file.slice(eol+1,i).replace(/^%%/gm,''))
...
s.wr=5
for(s2=s.prev;s2;s2=s2.prev){if(w_tb[s2.type]){if(s2.type==C.GRACE)
s.wl-=6
if(s2.type==C.STBRK)
s.wl-=12
break}}
...
function add_end_bar(s){var b=_bar(s)
b.wl=0
b.wr=0
b.ts_prev=s
b.next=s.next
...
return b}
function set_allsymwidth(){var val,st,s_chs,stup,s=tsfirst,s2=s,xa=0,xl=[],wr=[],maxx=xa,tim=s.time
...
if(s2.dur&&s2.dur!=s2.notes[0].dur){if(!stup){stup=s2
while(stup.prev&&stup.prev.dur&&stup.prev.dur!=stup.prev.notes[0].dur)
stup=stup.prev
while(!stup.seqst)
stup=stup.ts_prev}}else if(stup&&stup.v==s2.v){set_sp_tup(stup,s2)
stup=null}
if(!s2.shrink&&!s2.space&&s2.type==C.CLEF){delete s2.seqst;s2.time=tim}
tim=s2.time
if(!s)
break
...
function bardiv(so){var s,s1,s2,t1,t2,i,noseq
...
if(s.type!=C.BAR)
add_end_bar(s)}
function do_warn(s){var s1,s2,s3
...
continue
if(s2.type==C.CLEF){for(s1=s.ts_prev;s1;s1=s1.ts_prev){switch(s1.type){case C.BAR:case C.KEY:case C.METER:continue}
break}}
s3=clone(s2)
...
wmx+=next.wr
return[w,wmx]}
function set_lines(s,next,lwidth,indent){var first,s2,s3,s4,x,xmin,xmid,xmax,wwidth,shrink,space,nlines,last=next?next.ts_prev:null,ws=get_width(s,next)
...
xmax-=8;for(;s!=next;s=s.ts_next){if(s.dur){if(!s.beam_end)
beam|=1<<s.v
else
beam&=~(1<<s.v)}
x=s.x
...
if(beam||s.in_tuplet)
continue
if(s3&&s.x>=xmid){if(xmid-s3.x>s.x-xmid||(s.dur&&(s.time+s.dur-bar_time)%(C.BLEN/4)==0))
s3=s
break}
s3=s}}
if(!s3){s3=s=s4
...
if(x+s.wr>=xmax)
break
if(s3&&s.x>=xmid){if(xmid-s3.x>s.x-xmid)
s3=s
break}
s3=s}}
s=s3
...
break}
continue}
if(!s.ts_next){s=null}else if(!s.soln&&!img.chg){continue}else{s.soln=false
if(s.time==s2.time)
continue
while(!s.seqst)
s=s.ts_prev}
...
if(s.ntrem)
nflags+=s.ntrem
if(s.type==C.REST&&s.beam_end&&!s.beam_on){s.beam_end=false;start_flag=true}
...
if(curvoice&&curvoice.ignore&&info_type!='V')
return
...
if(!parse.part)
parse.part={}
if(parse.part[curvoice.time])
break
s={text:text,time:curvoice.time}
set_ref(s)
...
if(s.text){s.rbstop=2;s.rbstart=2
...
case"[]":case"[|]":s.invis=true;bar_type=s.rbstart?"[":"[]"
break
case":|:":case":||:":bar_type="::"
break
...
if(!c)
break
if(curvoice.ignore){while(1){if(c=='['&&line.buffer[line.index+1]=='V'&&line.buffer[line.index+2]==':')
break
c=line.next_char()
if(!c)
return}}
...
break}
var pplet=line.get_int(),qplet=qplet_tb[pplet],rplet=pplet,c=line.char()
...
tpn=tp.length
c=cfmt.tuplets
if(curvoice.pos.tup){c=Object.create(c)
switch(curvoice.pos.tup&0x07){case C.SL_ABOVE:c[3]=1
break
case C.SL_BELOW:c[3]=2
break
case C.SL_HIDDEN:c[2]=1
break}}
tp.push({p:pplet,q:qplet,r:rplet,ro:rplet,f:c})
continue}
...
return str
var o,n_font,wh,o_font=gene.curfont,c_font=o_font
...
function write_headform(lwidth){var c,font,font_name,align,x,y,sz,info_val={},info_font=Object.create(info_font_init),info_sz={A:cfmt.infospace,C:cfmt.composerspace,O:cfmt.composerspace,R:cfmt.infospace},info_nb={}
...
head+='black"';head+=' stroke-width=".7"'
if(cfmt.bgcolor)
head+=' style="background-color: '+cfmt.bgcolor+'"';font=get_font("music")
...
font_style=''
if(cfmt.fullsvg&&typeof document=="undefined"){defined_glyph={}
...
curvoice.eoln=false}
if(parse.part&&parse.part[tim]){if(!parse.part[tim].done){s.part=parse.part[tim]
...
return s}
var w_tb=new Uint8Array([5,1,8,0,4,2,3,9,9,0,9,4,0,7,0,0,0,0])
...
Abc.prototype.set_bar_num=function(){var s,s2,tim,rep_tim,k,n,bar_num=gene.nbar,bar_tim=0,ptim=0,wmeasure=voice_tb[cur_sy.top_voice].meter.wmeasure
...
for(;s;s=s.ts_next){if(!s.seqst)
continue
switch(s.type){case C.METER:if(wmeasure!=1)
...
break}}
function generate(in_mc){var s,v,p_voice;if(a_dcn.length){syntax(1,"Decoration without symbol")
a_dcn=[]}
...
if(vover.bar&&vover.time){sym_link({type:C.BAR,bar_type:vover.bar,invis:1,dur:0,multi:0})}}
...
function parse_gchord(type){var c,text,gch,x_abs,y_abs,type,i,j,istart,iend,ann_font=get_font("annotation"),h_ann=ann_font.size,line=parse.line
...
set_an_yl(i)}}
function xml2tree(xml){var i,j,item,n,no,e=0,stack=[],root={name:"root"},o=root
while(1){i=xml.indexOf('<',e)
if(i<0)
break
if(i>e+1){item=xml.slice(e+1,i).replace(/\s*/,'')
if(item){if(!o.children)
o.children=[]
o.children.push({name:"text",content:item})}}
e=xml.indexOf('>',i)
if(e<0)
return root
switch(xml[i+1]){case'?':continue
case'!':e=xml.indexOf('-->',i)
if(e<0)
return root
continue
case'/':item=xml.slice(i+2,e)
if(item!=o.name){error(1,null,'Found tag </'+item+'> instead of </'+o.name+'>')
continue}
o=stack.pop()
if(!o)
...
no={name:item.match(/[\w-]+/)[0],ix:i}
if(!o.children)
o.children=[]
o.children.push(no)
if(item.slice(-1)!='/'){stack.push(o)
o=no}else{item=item.slice(0,-1).trim()}
item=item.replace(/[^\s]*\s*/,'')
if(!item)
continue
n=item.match(/=|[^\s"=]+|".*?"/g)
for(j=0;j<n.length;j+=3){if(n[j+1]!='='||!n[j+2]){mus.error(1,null,'Bad key=value in "'+item+'"')
break}
no[n[j]]=n[j+2].slice(1,-1)}}
return root}
Abc.prototype.mei2mus=function(mei){var curr={beam:0,delayed:{},fnt:{},ftn:0,ids:{},meastim:0,mu:C.BLEN/4,pt:.01,tp:[]},acc_ty={"ff":-2,"f":-1,"s":1,"x":2,"n":3},bar_ty={dashed:".|",dbl:"||",dbldashed:".||",dbldotted:".||",dotted:".|",end:"|]",invis:"[]",rptstart:"|:",rptboth:"::",rptend:":|",single:"|"},fn="",fns={accid:function(tag){var s=curr.s,note=s.notes[s.nhd]
if(acc_ty[tag.accid])
note.acc=acc_ty[tag.accid]},arpeg:function(tag){var s
if(!tag.plist)
return
s=tag.plist.split(/\s+/)[0].slice(1)
s=curr.ids[s]
s=s.s
add_dcn(s,"arpeggio")},artic:function(tag){artic(curr.chord||curr.s,tag)},barLine:function(tag){var s,ty
if(tag.form)
ty=bar_ty[tag.form]
if(!ty)
ty="|"
s={type:C.BAR,istart:tag.ix,bar_type:ty,dur:0,multi:0}
sym_link(s)
if(ty[0]=='.'){s.bar_type=ty.slice(1)
s.bar_dotted=true}},beam:function(tag){curr.beam=1},beamSpan:function(tag){var s=get_ref(tag),s2=get_ref(tag,s)
if(!s||!s2)
return
s.beam_st=true
if(s.st==s2.st){while(s!=s2){if(s.type==C.BAR)
s.beam_on=true}}
s2.beam_end=true},breath:function(tag){var s=get_ref(tag)
if(!s)
return
if(s.s)
s=s.s
add_dcn(s,"breath")},chord:function(tag){var dur=durcnv(tag),s={type:C.NOTE,istart:tag.ix,stem:0,multi:0,nhd:-1,xmx:0,dur:dur,dur_orig:dur,notes:[]}
switch(tag["stem.dir"]){case"up":s.stem=1;break
case"down":s.stem=-1;break}
switch(curr.beam){case 0:s.beam_st=true
s.beam_end=true
break
case 1:s.beam_st=true
curr.beam=2
break}
if(tag.artic)
artic(s,tag)
...
s.grace=true
sym_link(s)
curr.chord=s},clef:function(tag){var s=new_clef(tag.shape+tag.line)
if(s)
get_clef(s)},composer:set_text,dir:set_text,div:set_text,dynam:set_text,ending:function(tag){curr.ending=tag.n},f:function(tag){if(curr.text)
curr.text+="\n"},fileDesc:function(tag){return true},fing:set_text,fermata:function(tag){var s=get_ref(tag)
if(!s)
return
if(s.s)
s=s.s
add_dcn(s,tag.form!="inv"?"fermata":"invertedfermata")},graceGrp:function(tag){},hairpin:function(tag){var s=get_ref(tag),s2=get_ref(tag,s)
if(!s||!s2)
return
if(s.s)
s=s.s
if(s2.s)
s2=s2.s
switch(tag.place){case"above":s.pos.dyn=C.SL_ABOVE;break
case"below":s.pos.dyn=C.SL_BELOW;break}
switch(tag.form){default:add_dcn(s,"<(")
add_dcn(s2,"<)")
break
case"dim":add_dcn(s,">(")
add_dcn(s2,">)")
break}},harm:set_text,incip:function(tag){return true},l:function(tag){if(curr.text)
curr.text+="\n"},label:function(tag){if(curr.st==undefined&&!curr.staffGrp)
return true
set_text(tag)},layer:function(tag){var v,p_v,old_sy,s,vid,t,st=curr.st,staff=par_sy.staves[st],n=tag.n
if(parse.state<2){staves_found=0
if(staff["meter.sym"]||staff["meter.count"]){new_meter(metercnv(staff))
...
if(!par_sy.rv)
par_sy.rv=[]
par_sy.rv.push({v:v,r:st*10+n})
par_sy.rv.sort(function(n1,n2){return n1.r-n2.r})
for(n=0;n<par_sy.rv.length;n++){v=par_sy.rv[n].v
par_sy.voices[v].range=n}
v=p_v.v
while(--v>=0){if(par_sy.voices[v].st==st){p_v.second=par_sy.voices[p_v.v].second=true
break}}}
if(tag.label){t=tag}else if(staff.label){t=staff.label
staff.label=null}
if(t){p_v.nm=t.label
p_v.new_name=true
if(t["label.abbr"])
...
get_voice(vid)
if(staff["clef.shape"]){s=new_clef(clefcnv(staff))
delete staff["clef.shape"]
if(s){get_clef(s)
if(s.prev&&s.prev.bar_type){s.next=s.prev
if(s.prev.prev)
s.prev.prev.next=s
else
s.p_v.sym=s
...
s.next.prev=s
s.next.next=null
curvoice.last_sym=s.next}}}},lb:function(tag){curr.text+="\n"},mei:function(tag){if(!tag.meiversion||tag.meiversion[0]!="4")
error(0,null,"Bad MEI version "+tag.meiversion)},meter:function(tag){var m
if(tag.count){m=tag.count
if(tag.unit)
m+="/"+tag.unit
new_meter(m)
m=C.BLEN/(tag.unit?Number(tag.unit):1)}else switch(tag.sym){case"common":new_meter("C");m=4;break
case"cut":new_meter("C|");m=2;break}
if(m)
curr.mu=C.BLEN/m},mRest:function(tag){var s={type:C.REST,istart:tag.ix,stem:0,multi:0,nhd:0,xmx:0,dur:curvoice.wmeasure,dur_orig:curvoice.wmeasure}
sym_link(s)
if(curvoice.wmeasure==1)
curr.mrest=true
if(par_sy.staves[s.st].invis)
s.invis=true},measure:function(tag){curr.meas=tag.n},mordent:function(tag){var ty,s=get_ref(tag)
if(!s)
return
if(s.s)
s=s.s
switch(tag.form){default:ty="uppermordent";break
case"lower":ty="lowermordent";break}
add_dcn(s,ty)},multiRest:function(tag){var dur=curvoice.wmeasure*tag.num,s={type:C.MREST,istart:tag.ix,stem:0,multi:0,nhd:0,xmx:0,dur:dur,dur_orig:dur}
sym_link(s)},music:function(tag){info.X="1"
parse.fname="mei"
parse.state=1},note:function(tag){var note,s,acc,pit,dur,v,i,vi,gr,ref,st,oct=tag.oct,id=tag["xml:id"]
if(!tag.pname){if(tag["pname.ges"]){tag.pname=tag["pname.ges"]}else if(tag.sameas){ref=tag.sameas
if(ref[0]=='#')
ref=ref.slice(1)
s=clone(curr.ids[ref])
if(!s.type){note=s
s=null}else{s.notes=clone(s.notes)
s.notes[0]=note=clone(s.notes[0])
note=s}}else{return}}
if(!note){if(!oct)
oct=curr.oct
note={pit:"cdefgab".indexOf(tag.pname.toLowerCase())+
oct*7-12,shhd:0,shac:0}}
if(curr.chord){s=curr.chord
dur=s.dur
s.notes[++s.nhd]=note}else{dur=durcnv(tag)
if(!s){s={type:C.NOTE,stem:0,multi:0,nhd:0,xmx:0,dur:dur,dur_orig:dur,notes:[note]}}
s.istart=tag.ix
switch(tag["stem.dir"]){case"up":s.stem=1;break
case"down":s.stem=-1;break}
if(tag.grace){gr=curvoice.last_sym
if(gr&&gr.type!=C.GRACE){gr={type:C.GRACE,istart:tag.ix,dur:0,stem:0,multi:0}
sym_link(gr)
gr.extra=s}else{gr=gr.extra
while(gr.next)
gr=gr.next
gr.next=s
s.prev=gr}
...
s.v=curvoice.v
s.p_v=curvoice}else{switch(curr.beam){case 0:s.beam_st=true
s.beam_end=true
break
case 1:s.beam_st=true
curr.beam=2
...
if(tag.staff)
s.st=curr.st_cnv[tag.staff-1]
if(tag.grace)
s.grace=true
switch(tag.tuplet){case"i1":curr.tp.push(s)
break
case"t1":if(curr.tp.length)
tp_set(curr.tp.pop(),{num:3,numbase:2})
break}
if(tag.color)
s.color=tag.color
curr.s=s
...
if(tag.artic)
artic(s,tag)
switch(tag.breaksec){case"1":s.beam_br1=true;break
case"2":s.beam_br2=true;break}},pb:function(tag){var s
if(parse.state>=2){s=new_block("newpage")
s.param=tag.n||""}else{blk_flush()
block.newpage=true}},pedal:function(tag){var dcn,s=get_ref(tag)
if(!s)
return
if(s.s)
s=s.s
switch(tag.dir){default:dcn="ped)";break
case"down":dcn="ped(";break}
add_dcn(s,dcn)},rend:function(tag){var font,n,f=""
if(tag.fontfam)
f+=" "+tag.fontfam
if(tag.fontname)
f+=" "+tag.fontname
...
f=f.slice(1)
if(tag.fontsize){n=parseInt(tag.fontsize)*1.33
f+=" "+n.toString()}else{f+=" *"}
if(f!="* *"){n=curr.fnt[f]
if(!n){n=++curr.ftn
curr.fnt[f]=n
...
curr.text+='$'+n
curr.rend=true}},rest:function(tag){var id=tag["xml:id"],dur=durcnv(tag),s={type:C.REST,istart:tag.ix,stem:0,multi:0,nhd:0,xmx:0,dur:dur,dur_orig:dur}
if(tag.name=="rest"){switch(curr.beam){case 0:s.beam_st=true
s.beam_end=true
break
case 1:s.beam_st=true
curr.beam=2
break}}else{s.invis=true}
sym_link(s)
if(id)
do_delayed(id,s)
switch(tag.tuplet){case"i1":curr.tp.push(s)
...
case"t1":if(curr.tp.length)
tp_set(curr.tp.pop(),{num:3,numbase:2})
break}},revisionDesc:function(tag){return true},sb:function(tag){voice_tb[0].eoln=true},scoreDef:function(tag){var w,h,v,tmp
curr.scoreDef=tag
if(parse.state>=2){if(tag["key.sig"]){tmp=keycnv(tag)
for(v=0;v<voice_tb.length;v++){curvoice=voice_tb[v]
get_key(tmp)}}}
if(tag["page.height"]&&tag["page.width"]){h=tag["page.height"]
w=tag["page.width"]
if(/^\d+$/.test(h+w))
curr.pt=h/w>1.3?1123.66/h:1056/h}
if(tag["page.leftmar"])
self.set_format("leftmargin",unitcnv(tag["page.leftmar"]))
if(tag["page.rightmar"])
self.set_format("rightmargin",unitcnv(tag["page.rightmar"]))
if(tag["page.scale"])
self.set_format("pagescale",tag["page.scale"])
if(tag["page.width"])
self.set_format("pagewidth",unitcnv(tag["page.width"]))},slur:function(tag){var sl,s1=get_ref(tag),s2=get_ref(tag,s1),ty=C.SL_AUTO
if(!s1||!s2)
return
switch(tag.curvedir){case"above":ty=C.SL_ABOVE;break
case"below":ty=C.SL_BELOW;break}
switch(tag.lform){case"dashed":case"dotted":ty|=C.SL_DOTTED;break}
sl={ty:ty}
if(s2.s){sl.is_note=true
sl.note=s2}else{s2.notes[0].s=s2
sl.note=s2.notes[0]}
if(!s1.sls)
s1.sls=[]
s1.sls.push(sl)
if(s1.s)
s1.s.sl1=true},sourceDesc:function(tag){return true},staff:function(tag){curr.st=curr.st_cnv[tag.n-1]
if(curr.st==undefined){error(1,null,"Unknown staff "+tag.n)
return true}
var staff=par_sy.staves[curr.st]
switch(tag.visible){case"false":staff.invis=true
break
case"true":staff.invis=false
break}},staffDef:function(tag){var k,st,def,staff,sy,n=tag.n-1,grp=curr.staffGrp
if(!curr.st_cnv)
curr.st_cnv=[]
else
st=curr.st_cnv[n]
if(st==undefined){staff={stafflines:'|||||',staffscale:1,flags:0}
if(grp){staff.flags=grp.flags
grp.flags&=STOP_BAR}
if(parse.state==3&&!curr.newsy){curr.newsy=true
sy=par_sy
par_sy=clone(sy)
sy.next=par_sy
par_sy.voices=clone(par_sy.voices)
par_sy.staves=clone(par_sy.staves)}
st=par_sy.staves.length
par_sy.staves.push(staff)
par_sy.nstaff=st
curr.st_cnv[n]=st}else{staff=par_sy.staves[st]}
def=curr.scoreDef
if(def){for(k in def){switch(k.slice(0,3)){case"cle":case"key":case"met":staff[k]=def[k]
break}}}
for(k in tag){switch(k.slice(0,3)){case"cle":case"key":case"met":staff[k]=tag[k]
break}}
if(tag.label){staff.label=tag}else if(grp&&grp.label){staff.label={label:grp.label}
delete grp.label}
if(tag.spacing){var v=tag.spacing.slice(-1)
if(v>='0'&&v<='9')
staff.maxsep=Number(tag.spacing)/100*CM
else
staff.maxsep=get_unit(tag.spacing)}
if(tag.children)
curr.st=st},staffGrp:function(tag){if(!tag.symbol)
return
var grp=curr.staffGrp
if(!grp){curr.staffGrp=grp={lvl:0,flags:0}
switch(tag.symbol){case"brace":grp.flags|=OPEN_BRACE;break
case"bracket":grp.flags|=OPEN_BRACKET;break}}else{grp.lvl++
switch(tag.symbol){case"brace":grp.flags|=OPEN_BRACE2;break
case"bracket":grp.flags|=OPEN_BRACKET2;break}}
if(tag["bar.thru"]=="false")
grp.flags|=STOP_BAR},syl:set_text,tempo:function(tag){set_text(tag)
if(!tag.children)
fne[tag.name](tag)},text:function(tag){if(curr.text!=undefined)
curr.text+=tag.content.replace(/[ \t\n]+/g,' ')},tie:function(tag){var m1,m2,not1,not2,s1=get_ref(tag),s2=get_ref(tag,s1),ty=C.SL_AUTO
if(!s1||!s2)
return
switch(tag.curvedir){case"above":ty=C.SL_ABOVE;break
case"below":ty=C.SL_BELOW;break}
...
not1.tie_ty=ty
not1.tie_e=not2
not2.tie_s=not1}else{for(m1=0;m1<=s1.nhd;m1++){not1=s1.notes[m1]
for(m2=0;m2<=s2.nhd;m2++){not2=s2.notes[m2]
if(not1.pit==not2.pit){not1.tie_ty=ty
not1.tie_e=not2
not2.tie_s=not1}}}
s1.ti1=s2.ti2=true},trill:function(tag){var s2,s=get_ref(tag)
if(!s)
return
if(tag.dur||tag["dur.ges"]||tag.endid||tag.tstamp2){s2=get_ref(tag,s)
if(!s2)
return}
if(s.s)
s=s.s
switch(tag.place){case"above":s.pos.orn=C.SL_ABOVE;break
case"below":s.pos.orn=C.SL_BELOW;break}
if(!s2){add_dcn(s,"trill")
return}
if(s.time>=s2.time){error(1,null,"Bad trill endings")
return}
add_dcn(s,"trill(")
add_dcn(s2,"trill)")},tuplet:function(tag){curr.tp.push(curvoice.last_sym||true)},tupletSpan:function(tag){var last,next,f,s=get_ref(tag),s2=get_ref(tag,s)
if(!s||!s2)
return
if(s.tp.length){tp_fl(s.tp[0].f,tag)
return}
...
tp_set(s,tag)
s2.next=next
curvoice.last_sym=last},verse:function(tag){curr.verse=tag},work:function(tag){curr.work=true},},fne={beam:function(tag){curr.s.beam_end=true
curr.beam=0},chord:function(tag){var id=tag["xml:id"]
if(id)
do_delayed(id,curr.chord)
curr.chord=null},composer:function(tag){if(!curr.text)
return
if(info.C==undefined)
info.C=curr.text
else
info.C+="\n"+curr.text
curr.text=undefined},dir:function(tag){var gch,ty,t,i,font=gene.deffont,s=get_ref(tag)
if(!s)
return
if(s.s)
s=s.s
switch(tag.name){case"dir":case"dynam":switch(tag.place){default:ty="^";break
case"below":ty="_";break}
break
default:ty="g"
switch(tag.place){default:s.pos.gch=C.SL_ABOVE;break
...
if(!s.a_gch)
s.a_gch=[]
for(i=0;i<t.length;i++){gch={type:ty,font:font,text:t[i]}
s.a_gch.push(gch)}
curr.text=undefined},div:function(tag){var s
switch(tag.type){case undefined:s=new_block("text")
s.text=curr.text
break
case"verse":if(info.W==undefined)
info.W=""
if(tag.label)
info.W+=tag.label+" "
info.W+=curr.text+"\n\n"
break
default:error(1,null,"Tag <div> type "+tag.type+"not treated")
break}
curr.text=undefined},dynam:function(tag){var t,s=get_ref(tag)
if(!s)
return
if(s.s)
s=s.s
...
case"below":s.pos.dyn=C.SL_BELOW;break}
add_dcn(s,t,tag)
curr.text=undefined},ending:function(tag){var v=voice_tb.length
while(--v>=0){if(voice_tb[v].last_sym)
voice_tb[v].last_sym.rbstop=2}},fing:function(tag){var s=get_ref(tag)
if(!s)
return
if(s.s)
s=s.s
...
case"below":s.pos.orn=C.SL_BELOW;break}
add_dcn(s,curr.text)
curr.text=undefined},label:function(tag){if(!curr.text)
return
var grp=curr.staffGrp,st=curr.st
if(st==undefined){if(grp){grp.label=curr.text
curr.text=undefined}
return}
...
curr.text=undefined},labelAbbr:function(tag){if(!curr.text)
return
var st=curr.st,staff=par_sy.staves[st]
if(!staff.label)
staff.label={}
staff.label["label.abbr"]=curr.text
curr.text=undefined},measure:function(tag){var v,s,s2,s3,p_v,tim,ty,dotted,n=voice_tb.length
if(curr.tp.length){error(1,null,"No end of tuplet")
curr.tp=[]}
if(tag.left){ty=bar_ty[tag.left]
if(ty&&ty[0]=='.'){ty=ty.slice(1)
dotted=true}}
if(ty||curr.ending){for(v=0;v<n;v++){p_v=voice_tb[v]
s2=s=p_v.last_sym
if(s)
s=s.prev
while(s&&!s.bar_type)
...
curvoice=p_v
sym_link({type:C.BAR,istart:tag.ix,bar_type:ty||"|",dur:0,multi:0})
s=p_v.last_sym
p_v.last_sym=s2
s.next=s3
if(s3)
s3.prev=s
s.time=curr.meastim}else if(ty){s.bar_type=ty}
if(dotted)
s.bar_dotted=true
if(curr.ending){s.rbstop=2
s.rbstart=2
s.text=curr.ending}}
dotted=false
ty=null
...
if(!ty)
ty="|"
if(curr.newsy){curr.newsy=false
for(s=voice_tb[par_sy.top_voice].last_sym;s;s=s.prev){if(s.type==C.BAR)
break}
if(s){s2=clone(s)
s2.type=C.STAVES
s2.bar_type=0
...
if(p_v.time>tim)
tim=p_v.time}
if(curr.mrest){curr.mrest=false
for(v=0;v<n;v++){p_v=voice_tb[v]
s=p_v.last_sym
if(s&&s.type==C.BAR)
s=s.prev
...
sym_link(s)}
curr.meastim=tim},music:function(tag){sanitize()
generate()
if(info.W)
put_words(info.W)
put_history()
blk_flush()},rend:function(tag){if(curr.rend){curr.rend=false
curr.text+="$0"}},scoreDef:function(tag){curr.scoreDef=null},staffDef:function(tag){delete curr.st},staffGrp:function(tag){var grp=curr.staffGrp,st=par_sy.staves.length-1,staff=par_sy.staves[st]
if(!grp)
return
if(--grp.lvl<0){curr.staffGrp=null
switch(tag.symbol){case"brace":staff.flags|=CLOSE_BRACE;break
case"bracket":staff.flags|=CLOSE_BRACKET;break}
return}
switch(tag.symbol){case"brace":staff.flags|=CLOSE_BRACE2;break
case"bracket":staff.flags|=CLOSE_BRACKET2;break}},syl:function(tag){var t=curr.text.replace(/[ \t\n]+/g,' ')
curr.text=undefined
if(t[0]==' ')
t=t.slice(1)
var l,wh,c,s=curr.s
...
if(t.slice(-1)=='-')
t=t.slice(0,-1)+'\n'
switch(tag.con){case"s":t+=' '
break
case"d":t+='-'
break
case"u":t+='_'
break}
switch(tag.wordpos){case"i":case"m":if(t.slice(-2)!="\n"){if(c=='_')
t+=c
t+="\n"}
break}
if(!s.a_ly)
s.a_ly=[]
s.a_ly[l]={t:t,font:gene.curfont,wh:wh}},tempo:function(tag){var t=curr.text.replace(/[ \t\n]+/g,' ')
curr.text=undefined
var s,s2={type:C.TEMPO,dur:0}
if(t)
s2.tempo_str1=t
if(tag.mm){s2.tempo_notes=[curr.mu]
s2.tempo=tag.mm}
if(parse.state<=1){info.Q='"'+t+'"'
glovar.tempo=s2
return}
s=get_ref(tag)
if(!s)
...
s2.p_v=s.p_v
s2.st=s.st
s2.time=s.time},title:function(tag){var t=curr.text.replace(/[ \t]+/g,' ')
curr.text=undefined
if(t[0]==' ')
t=t.slice(1)
if(parse.state!=3){if(info.T==undefined)
info.T=t
else
info.T+="\n"+t}else{var s=new_block("title")
s.text=t}},tuplet:function(tag){var s=curr.tp.pop()
if(s&&(tag.dur||tag.num))
tp_set(s==true?curvoice.sym:s.next,tag)},work:function(tag){curr.work=false},}
fns.lg=fns.l
fns.labelAbbr=fns.label
fns.space=fns.rest
fns.title=fns.tempo
fne.harm=fne.dir
function add_dcn(s,dcn,tag){var dd=dd_tb[dcn]
function build_dyn(t){var i,str="",dd={name:t,func:6,glyph:t,h:18,wl:t.length*.3,wr:t.length*.7}
for(i=0;i<t.length;i++){switch(t[i]){case'f':str+="\ue522";break
case'p':str+="\ue520";break
case'r':str+="\ue523";break
case's':str+="\ue524";break}}
tgls[t]={x:-4,y:-6,c:str}
return dd}
if(!dd){if(decos[dcn]){dd=deco_def(dcn)}else{if(!tag)
return
if(tag.name!="dynam"||/[^fprs]/.test(dcn)){fne.dir(tag)
return}
dd_tb[dcn]=dd=build_dyn(dcn)}}
if(!s.a_dd)
s.a_dd=[]
s.a_dd.push(dd)}
function artic(s,tag){var dcn,i,d_tb={acc:"accent",dnbow:"downbow",marc:"marcato",open:"open",snap:"snap",spic:"wedge",stacc:"dot",ten:"tenuto",upbow:"upbow",},d=tag.artic.split(/\s+/)
switch(tag.place){case"above":s.pos.orn=C.SL_ABOVE;break
case"below":s.pos.orn=C.SL_BELOW;break}
for(i=0;i<d.length;i++){dcn=d_tb[d[i]]
if(dcn)
add_dcn(s,dcn)
else
error(1,null,"Articulation "+d[i]+" not handled")}}
function do_delayed(id,s){var i,n,tag,delayed_a=curr.delayed[id]
curr.ids[id]=s
if(!delayed_a)
return
curr.delayed[id]=null
n=delayed_a.length
for(i=0;i<n;i++){tag=delayed_a[i]
fns[tag.name](tag)}}
function clefcnv(tag){var clef={"G":"G","GG":"G","F":"F","C":"C","perc":"P","TAB":"tab"}[tag["clef.shape"]]
if(tag["clef.line"])
clef+=tag["clef.line"]
if(tag["clef.dis"]){if(tag["clef.dis.place"]=="above")
clef+='+'+tag["clef.dis"]
else
clef+='-'+tag["clef.dis"]}
return clef}
function durcnv(tag){var dur
switch(tag.dur){case"breve":dur=C.BLEN*2;break
case"long":dur=C.BLEN*4;break
case undefined:dur=C.BLEN/4;break
default:dur=C.BLEN/tag.dur}
switch(tag.dots){case"1":dur*=3/2;break
...
return dur}
var err_old
function error_new(sev,s,msg,a1,a2,a3,a4){if(!s)
s={}
if(!s.fname)
s.fname="(no ref)"
if(!s.istart)
...
err_old=error
error=error_new
function get_ref(tag,end){var s,st,v_rg,v,voice,tim,rg=100,ref=tag[end?"endid":"startid"]
if(ref){if(ref[0]=='#')
ref=ref.slice(1)
s=curr.ids[ref]
if(!s){if(!curr.delayed[ref])
curr.delayed[ref]=[]
curr.delayed[ref].push(tag)
return}}else{ref=end?tag.tstamp2:tag.tstamp
if(!ref){error(1,null,"No "+(end?"end":"start")+" reference tag:"+tag.name)
return}
if(ref[1]=='m'){if(ref[0]=='0'&&ref[2]=='+')
ref=ref.slice(3)
else
return}
tim=curr.meastim+curr.mu*(ref-1)
st=curr.st_cnv[tag.staff?(tag.staff-1):0]
for(v=0;v<par_sy.voices.length;v++){voice=par_sy.voices[v]
if(!voice||voice.st!=st)
continue
if(voice.range<rg){v_rg=v
rg=voice.range}}
if(rg>=100){error(1,null,"No voice in staff "+st+" tag:"+tag.name)
return}
for(s=voice_tb[v_rg].last_sym;s;s=s.prev){if(s.time<=tim){if(ref!=0)
break
while(s&&s.type!=C.BAR)
s=s.prev
break}}}
if(!s){error(1,null,"No symbol at beat "+ref+" in staff "+tag.staff+" tag:"+tag.name)
return}
if(!end)
return s
if(s==end){error(1,s,"References on a same element")
if(!s.next)
return
s=s.next}
if(s.time<=end.time){error(0,s,"References going back in time")
return}
return s}
function keycnv(tag){var sig2kM={"0":"C","1s":"G","2s":"D","3s":"A","4s":"E","5s":"B","6s":"F#","7s":"C#","1f":"F","2f":"Bb","3f":"Eb","4f":"Ab","5f":"Db","6f":"Gb","7f":"Cb"},sig2km={"0":"Am","1s":"Em","2s":"Bm","3s":"F#m","4s":"C#m","5s":"G#m","6s":"D#m","7s":"A#m","1f":"Dm","2f":"Gm","3f":"Cm","4f":"Fm","5f":"Bbm","6f":"Ebm","7f":"Abm"}
return tag["key.mode"]!="minor"?sig2kM[tag["key.sig"]]:sig2km[tag["key.sig"]]}
function metercnv(tag){switch(tag["meter.sym"]){case"common":return"C"
case"cut":return"C|"}
if(tag["meter.unit"])
return tag["meter.count"]+"/"+tag["meter.unit"]
return tag["meter.count"]}
function meteru(tag){var mu=1
switch(tag["meter.sym"]){case"common":mu=4;break
case"cut":mu=2;break}
if(tag["meter.unit"])
mu=Number(tag["meter.unit"])
return C.BLEN/mu}
function set_text(tag){var f
switch(tag.name){case"dir":f="annotation";break
...
set_font(f)
curr.text=""}
function tp_fl(f,tag){if(tag["num.visible"]=="false"){f[0]=1}else{if(tag["bracket.visible"]=="false")
f[1]=2
if(tag["num.format"]=="ratio")
f[2]=2}
switch(tag["bracket.place"]){case"above":f[3]=1;break
case"below":f[3]=2;break}}
function tp_set(s,tag){var fact,s2,s3,n,nbas=tag.numbase||2,f=Object.create(cfmt.tuplets)
while(s.grace)
s=s.next
if(!s.tp)
s.tp=[]
s.tp.push({p:tag.num,q:nbas,f:f})
tp_fl(f,tag)
if(tag.dur)
...
if(!s2.dur){for(s3=s2;!s3.dur;s3=s3.prev);s3.tpe=s2.tpe
delete s2.tpe}
n=0
for(s3=s;s3;s3=s3.next){if(s3.dur)
n++}
s.tp[s.tp.length-1].ro=n}
function unitcnv(v){var nv=v.slice(-1)
if(nv>='0'&&nv<='9')
return(v*curr.pt).toString()
return v}
function sanitize(){var i,v,used_st=[]
for(v=0;v<voice_tb.length;v++){i=voice_tb[v].st
if(i)
used_st[i]=true}
for(i=par_sy.nstaff;i>0;i--){if(used_st[i])
break}
if(i!=par_sy.nstaff)
error(1,null,"Unused staves "+(i+1)+".."+par_sy.nstaff)
par_sy.nstaff=i}
function parse_mei(tag){if(fns[tag.name]){parse.istart=tag.ix
if(fns[tag.name](tag))
return}
if(!tag.children)
return
var i,n=tag.children.length
for(i=0;i<n;i++)
...
abc2svg.modules.cbf()}
function load_ko(fn){abc2svg.modules.errmsg('Error loading the module '+fn)
load_end()}
...
fn=m.fn
this.nreq++
abc2svg.loadjs(fn+"-1.js",load_end,load_ko)}
return this.nreq==nreq_i}}
abc2svg.version="1.21.7";abc2svg.vdate="2022-04-04"
