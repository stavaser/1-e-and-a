...
abc2svg.gamelan={cde2fcg:new Int8Array([0,2,4,-1,1,3,5]),cgd2cde:new Int8Array([0,-4,-1,-5,-2,-6,-3,0,-4,-1,-5,-2,-6,-3,0]),acc2:new Int8Array([-2,-1,3,1,2]),do_pscom:function(of,p){switch(p.match(/\w+/)[0]){case'staves':case'score':p=p.replace(/\(|\)/g,'')
break}
of(p)},output_music:function(of){var C=abc2svg.C,abc=this,cur_sy=abc.get_cur_sy(),voice_tb=abc.get_voice_tb()
if(!abc.cfmt().gamelan){of()
return}
function slice(s){var m,n,s2,s3,d,d_orig
if(s.dur<=C.BLEN*3/8){if((s.dur_orig/9|0)*9!=s.dur_orig)
return
d=s.dur/3
d_orig=s.dur_orig/3
s.dur-=d
s.dur_orig-=d_orig
n=1}else{if(s.dur>=C.BLEN)
n=3
...
s.notes[m].dur=s.dur
s.beam_on=true
while(--n>=0){s2={type:C.REST,v:s.v,p_v:s.p_v,st:s.st,fmt:s.fmt,dur:d,dur_orig:d_orig,stem:1,multi:0,nhd:0,notes:[{pit:s.notes[0].pit,jn:8}],xmx:0,beam_on:true,noplay:true,time:s.time+s.dur,prev:s,next:s.next}
s.next=s2
if(s2.next)
s2.next.prev=s2
...
break}}
s=s2}}
function do_tie(s){var end_time=s.time+s.dur
while(1){s=s.ts_next
if(!s||s.time>end_time)
return
if(s.type==C.NOTE&&s.time==end_time)
break}
s.notes[0].jn=8
s.notes[0].jo=2}
function set_sym(p_v){var s,s2,note,pit,nn,p,a,m,i,sf=p_v.key.k_sf,delta=abc2svg.gamelan.cgd2cde[sf+7]-2
p_v.key.k_a_acc=[]
p_v.clef.invis=true
for(s=p_v.sym;s;s=s.next){s.st=p_v.st
switch(s.type){case C.CLEF:s.invis=true
default:continue
case C.KEY:delta=abc2svg.gamelan.cgd2cde[s.k_sf+7]-2
continue
...
p=note.pit
pit=p+delta
if(note.jn==undefined){note.jn=((pit+77)%7)+1
note.jo=(pit/7)|0}
note.pit=21
a=note.acc
if(a){nn=abc2svg.gamelan.cde2fcg[(p+5+16*7)%7]-sf
if(a!=3)
nn+=a*7
nn=((((nn+1+21)/7)|0)+2-3+32*5)%5
note.acc=abc2svg.gamelan.acc2[nn]}
if(note.tie_ty){do_tie(s)
delete note.tie_ty}}
slice(s)
...
if(!abc.cfmt().gamelan){of(p_voice)
return}
...
out_sxsy(x,'" y="',y)
out_svg('">'+p+'</text>\n')}
function out_txt(x,y,p){out_svg('<text class="bn" x="')
out_sxsy(x,'" y="',y)
out_svg('">'+p+'</text>\n')}
function draw_hd(s,x,y){var m,note,ym
for(m=0;m<=s.nhd;m++){note=s.notes[m]
out_txt(x-3.5,y+8,"01234567."[note.jn])
if(note.acc){out_svg('<path class="stroke" stroke-width="1.1" d="M')
if(note.acc>0){out_sxsy(x-6,' ',y+10)
out_svg("l12 -6")}else{out_sxsy(x-6,' ',y+16)
out_svg("l12 6")}
out_svg('"/>\n')}
if(note.jo>2){out_mus(x-1,y+23,dot)
if(note.jo>3){y+=3
out_mus(x-1,y+23.4,dot)}}else if(note.jo<2){ym=y+4
out_mus(x-1,ym,dot)}
y+=20}}
for(s=p_voice.sym;s;s=s.next){if(s.invis)
continue
switch(s.type){case C.NOTE:case C.REST:x=s.x
y=staff_tb[s.st].y
draw_hd(s,x,y)
if(s.nflags>0){if(s.beam_st){nl=s.nflags
s2=s
...
break}}},set_fmt:function(of,cmd,param){if(cmd=="gamelan"){var cfmt=this.cfmt()
...
if(!last_s||!this.cfmt().gamelan)
return
var C=abc2svg.C
for(var s=this.get_tsfirst();s;s=s.ts_next){switch(s.type){case C.NOTE:s.ymx=20*s.nhd+(s.nflags>0?30:24)
...
switch(s.type){case C.CLEF:case C.KEY:case C.METER:s.wl=s.wr=0
break}},set_hooks:function(abc){abc.do_pscom=abc2svg.gamelan.do_pscom.bind(abc,abc.do_pscom)
