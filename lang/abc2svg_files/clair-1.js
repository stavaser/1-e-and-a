...
abc2svg.clair={hl_tb:[new Int8Array([-10,-8,-6,-4,-2]),new Int8Array([-10,-9,-8,-6,-4,-2]),new Int8Array([-8,-6,-4,-2]),new Int8Array([-8,-6,-4,-2]),new Int8Array([-8,-7,-6,-4,-2]),new Int8Array([-6,-4,-2]),new Int8Array([-6,-4,-2]),new Int8Array([-6,-4,-2]),new Int8Array([-6,-5,-4,-2]),new Int8Array([-4,-2]),new Int8Array([-4,-2]),new Int8Array([-4,-2]),new Int8Array([-4,-3,-2]),new Int8Array([-2]),new Int8Array([-2]),new Int8Array([-2]),new Int8Array([-2,-1]),null,null,null,new Int8Array([1]),null,null,null,new Int8Array([3,4]),new Int8Array([4]),new Int8Array([4]),new Int8Array([4]),new Int8Array([4,5]),null,null,null,new Int8Array([7]),null,null,null,new Int8Array([9,10]),new Int8Array([10]),new Int8Array([10]),new Int8Array([10]),new Int8Array([10,11]),new Int8Array([10,11]),new Int8Array([10,12]),new Int8Array([10,12]),new Int8Array([10,12,13]),new Int8Array([10,12,13]),new Int8Array([10,12,14]),new Int8Array([10,12,14]),new Int8Array([10,12,14,15]),new Int8Array([10,12,14,15]),new Int8Array([10,12,14,16])],draw_hl:function(of,s){if(!s.p_v.clair){of(s)
return}
var i,m,hl,dx=s.grace?4:[4.7,5,6,7.2,7.5][s.head]*1.4,p_st=this.get_staff_tb()[s.st]
for(m=0;m<=s.nhd;m++){hl=abc2svg.clair.hl_tb[s.notes[m].pit]
if(!hl)
continue
for(i=0;i<hl.length;i++)
this.set_hl(p_st,hl[i],s.x,-dx,dx)}},draw_keysig:function(of,x,s){if(!s.p_v.clair){of(x,s)
return}
var i,staffb=this.get_staff_tb()[s.st].y,a_tb=new Int8Array(24),sc=[0,2,4,5,7,9,11,12,14,16,17,19,21,23],bn_tb=[7,2,-3,4,-1,6,1,8,3,-2,5,0,7,2,-3],bn=bn_tb[s.k_sf+7]
if(!s.k_a_acc){if(s.k_sf>0){for(i=1;i<=s.k_sf;i++){a_tb[bn_tb[i]+3]=1;a_tb[bn_tb[i]+12+3]=1}
if(bn+sc[s.k_mode]>8)
bn-=12
for(i=7;--i>=0;){y=bn+sc[s.k_mode+i]
this.xygl(x,staffb+y*3,(y&1)?"wk":"bk");if(a_tb[y+3])
this.xygl(x,staffb+y*3,"sht");switch(s.k_mode+i){case 3:case 7:case 10:x+=4.5
break}}}else{for(i=s.k_sf;i<0;i++){a_tb[bn_tb[i+6]+3]=1;a_tb[bn_tb[i+6]+12+3]=1}
if(bn+sc[s.k_mode]>8)
bn-=12
for(i=0;i<7;i++){y=bn+sc[s.k_mode+i]
this.xygl(x,staffb+y*3,(y&1)?"wk":"bk");if(a_tb[y+3])
this.xygl(x,staffb+y*3,"flt");switch(s.k_mode+i){case 2:case 6:case 9:x+=4.5
break}}}}else if(s.k_a_acc.length){for(i=0;i<s.k_a_acc.length;i++){var acc=s.k_a_acc[i];j=sc[(acc.pit+3)%7];a_tb[j]=acc.acc;a_tb[j+12]=acc.acc}}},new_clef:function(s){switch(s.clef_type){case't':s.clef_line=3;s.clefpit=-2;s.clef_name="cl4"
break
case'c':s.clef_line=3;s.clefpit=6;s.clef_name="cl3"
break
case'b':s.clef_line=3;s.clefpit=14;s.clef_name="cl2"
break}},output_music:function(of){var s,m,mp,p_v,v,cfmt=this.cfmt(),tsfirst=this.get_tsfirst(),voice_tb=this.get_voice_tb()
...
if(!p_v.clair)
continue
p_v.scale=1.3;for(s=p_v.sym;s;s=s.next){if(!s.dur)
continue
for(m=0;m<=s.nhd;m++){mp=s.notes[m].midi
if(mp){mp-=46;s.notes[m].pit=mp
delete s.notes[m].acc}}}}
of()},set_fmt:function(of,cmd,param){if(cmd=="clairnote"){if(!this.get_bool(param))
return
this.set_v_param("clair",true);this.set_v_param("stafflines","|-|---|-|")
this.set_v_param("staffscale",.8)
return}
of(cmd,param)},set_pitch:function(of,last_s){if(last_s){of(last_s)
return}
var p_v,s,v,voice_tb=this.get_voice_tb(),staff_tb=this.get_staff_tb()
for(v=0;v<voice_tb.length;v++){p_v=voice_tb[v]
if(!p_v.clair)
continue
abc2svg.clair.new_clef(staff_tb[p_v.st].clef)
for(s=p_v.sym;s;s=s.next){if(s.clef_type)
abc2svg.clair.new_clef(s)}}
of(last_s)},set_vp:function(of,a){var i,curvoice=this.get_curvoice()
for(i=0;i<a.length;i++){if(a[i]=="clair="){curvoice.clair=a[i+1]
break}}
of(a)},set_width:function(of,s){if(s.k_sf&&s.p_v&&s.p_v.clair){s.wl=8;s.wr=10}else{of(s)}},set_hooks:function(abc){abc.draw_hl=abc2svg.clair.draw_hl.bind(abc,abc.draw_hl);abc.draw_keysig=abc2svg.clair.draw_keysig.bind(abc,abc.draw_keysig);abc.output_music=abc2svg.clair.output_music.bind(abc,abc.output_music);abc.set_format=abc2svg.clair.set_fmt.bind(abc,abc.set_format);abc.set_pitch=abc2svg.clair.set_pitch.bind(abc,abc.set_pitch);abc.set_vp=abc2svg.clair.set_vp.bind(abc,abc.set_vp);abc.set_width=abc2svg.clair.set_width.bind(abc,abc.set_width)}}
abc2svg.modules.hooks.push(abc2svg.clair.set_hooks);abc2svg.modules.clairnote.loaded=true
