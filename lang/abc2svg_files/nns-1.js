...
abc2svg.nns={note_nm:"CDEFGAB",nns_nm:["1","♯1","2","♭3","3","4","♯4","5","♯5","6","♭7","7"],nns_nm_m:["1","♭2","2","3","♯3","4","♭5","5","6","♯6","7","♯7"],rnn_nm:["I","♯I","II","♭III","III","IV","♯IV","V","♯V","VI","♭VII","VII"],rnn_nm_m:["I","♭II","II","III","♯III","IV","♭V","V","VI","♯VI","VII","♯VII"],block_gen:function(of,s){if(s.subtype!="nns"){of(s)
return}
...
abc.blk_flush()
build_nns(s,font)
abc.blk_flush()},set_stems:function(of){var C,tsfirst,voice_tb,fmt,p_v,s,s2,mode,abc=this,nns=abc.cfmt().nns
...
beat=C.BLEN/8*3
return beat}
function set_nm(p,tr){var i,o,o2,a,n,csa=[]
i=p.indexOf('/')
...
if(mode)
n=(n+3)%12
if(!nns.roman){if(!mode)
n=abc2svg.nns.nns_nm[n]
else
n=abc2svg.nns.nns_nm_m[n]}else{if(!mode)
...
+p.slice(o2)}
return csa.join('/')}
function build_chords(sb){var s,i,w,bt,rep,bars=[],chords=[],parts=[],chord=[],beat=get_beat(voice_tb[0].meter),wm=voice_tb[0].meter.wmeasure,cur_beat=0,beat_i=0,tr=(tsfirst.p_v.key.k_sf+12)*5
bars.push('|')
...
break
for(i=0;i<s.a_gch.length;i++){if(s.a_gch[i].type=='g'){if(!chord[beat_i]){chord[beat_i]=set_nm(s.a_gch[i].text,tr)}
break}}
break
