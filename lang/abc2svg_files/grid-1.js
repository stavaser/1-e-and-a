...
function build_grid(s,font){var i,k,l,nr,bar,w,hr,x0,x,y,yl,ps,lc='',chords=s.chords,bars=s.bars,parts=s.parts||[],wmx=s.wmx,cells=[],nc=grid.n
...
abc.blk_flush()
build_grid(s,font)
abc.blk_flush()},set_stems:function(of){var C=abc2svg.C,abc=this,tsfirst=abc.get_tsfirst(),voice_tb=abc.get_voice_tb(),grid=abc.cfmt().grid
...
switch(s.type){case C.NOTE:case C.REST:if(!s.a_gch||chord[beat_i])
break
bt=abc2svg.cs_filter(s.a_gch,abc.cfmt().altchord)
if(!bt)
break
chord[beat_i]=bt
for(i=0;i<s.a_gch.length;i++){if(s.a_gch[i].type=='g')
break}
...
bt=s.bar_type
while(s.ts_next&&s.ts_next.time==s.time){if(s.ts_next.dur)
break
s=s.ts_next
